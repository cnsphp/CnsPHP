...
class Controller {
    ...
    private function parseURI(){
        ...
        $arr=preg_split('/\//',$this->uri);

        $this->module=$this->replaceGap($arr[0]);
        $this->controller=$this->replaceGap($arr[1]);
        $this->method = $this->replaceGap($arr[2]);
    ...
    private function replaceGap($str){ 
        if(strpos($str,'-') !== FALSE)
    ...
    public function show($tpl="") {
        if($tpl == "")
            $this->view->display(strtolower($this->module.'/'.$this->controller.'/'.$this->method.'.html'));
        else
    ...
    public function strClass() {
        return $this->str = new class {
            ...
            public function echoln($msg) {
        		echo "$msg\n";
        	}
        
        	public function randstr($len = 8, $format = 'ALL', $special='~!@#$%^&*_-+=,.?;:')
        	{
        		$is_abc = $is_numer = 0;
        		$password = $tmp = '';
        		switch ($format) {
    ...
    function fileClass(){
        return $this->file = new class {
            ...
            public function upload($upfiles,$dst,$types,$exts,$maxsize=204800) {
